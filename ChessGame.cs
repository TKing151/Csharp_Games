...
namespace PA6_Draft
{
    ...
    class Move
    {
        ...
        public override string ToString()//we are making a algebraic notation for the
        {
            ...
            {
                System.Media.SoundPlayer player = new SoundPlayer(PA6_Draft.Properties.Resources.PieceEat);
            ...
            {
                System.Media.SoundPlayer player = new SoundPlayer(PA6_Draft.Properties.Resources.Move);
            }
            result += (char)('a' + X2);
            result += (8 - Y2);
            switch (Promoted)
            ...
            if (Check)
            { result += "+";
                System.Media.SoundPlayer player = new SoundPlayer(PA6_Draft.Properties.Resources.Check);

            }
            if (Checkmate)
            { result += ("#" + (((int)MovedPiece % 2 == 0) ? "0-1" : "1-0"));
                System.Media.SoundPlayer player = new SoundPlayer(PA6_Draft.Properties.Resources.GameOver);
            }
            if (Stalemate)
            { result += "1/2-1/2";
                System.Media.SoundPlayer player = new SoundPlayer(PA6_Draft.Properties.Resources.GameOver);
